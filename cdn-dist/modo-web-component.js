...
class CustomerData{_uniqueId;_userData;modo;phoneNumber;constructor(e,t){this.modo=e,this.initializeUniqueId(),this.updateUserData(t),this.initializePhoneNumber();}initializePhoneNumber(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-phone-number`);e&&(this.phoneNumber=e);}initializeUniqueId(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-unique-id`);this._uniqueId=e||crypto.randomUUID();}get uniqueId(){return this._uniqueId}get userData(){return this._userData||{}}async updateUserData(e){e&&"object"==typeof e?(this._userData=e,await this.fetchUpdate()):e&&console.warn("Invalid user data");}hasSubmittedPhoneForm(){return Boolean(this.phoneNumber)}savePhoneNumber(e){this.phoneNumber=e||"no phone number",localStorage.setItem(`modo-chat:${this.modo.publicKey}-user-phone-number`,e||"no phone number");}async fetchUpdate(){await fetchUpdateUserData(this.modo.publicData?.setting.uuid,this.uniqueId,this.userData);}}

class ModoChat{container;publicKey;publicData;customerData;conversation;socket;options;openedCount=0;version;isInitialized=false;isOpen=false;constructor(t,o){this.publicKey=t,this.customerData=new CustomerData(this,o?.userData),this.version=VERSION,this.options={position:o?.position||"right",theme:o?.theme||"dark",primaryColor:o?.primaryColor||"#667eea",title:o?.title||"",userData:o?.userData,foregroundColor:o?.foregroundColor||"#fff",fullScreen:"boolean"==typeof o?.fullScreen&&o?.fullScreen},o?.autoInit&&this.init();}async init(){if(this.isInitialized)throw new Error("ModoChat already initialized");const t=await fetchModoPublicData(this.publicKey);if(this.publicData=new ModoPublicData(t),!checkIfHostIsAllowed(this))throw new Error("host not allowed");if(await loadCss(),window.modoChatInstance=()=>this,createChatContainer(this),applyModoOptions(),loadStarters(),updateChatToggleImage(),updateChatTitle(),loadConversation(this),this.isInitialized=true,this.options.fullScreen){const t=this.container?.querySelector(".mc-chat-body");t&&(t.classList.remove("mc-hidden"),t.classList.add("mc-active")),this.onOpen();}}async onOpen(){if(this.isOpen=true,this.openedCount++,this.conversation?.hideTooltip(),this.conversation?.markAsRead(),this.conversation?.scrollToBottom(),this.options.fullScreen){const t=this.container?.querySelector(".mc-chat-body");t&&(t.classList.remove("mc-hidden"),t.classList.add("mc-active"));}1===this.openedCount&&(this.conversation&&(await(this.conversation?.loadMessages()),await initSocket()),await this.customerData.fetchUpdate());}onClose(){this.isOpen=false;}async updateUserData(t){await this.customerData.updateUserData(t);}}window.ModoChat=ModoChat;

...
const getEnvironment=()=>"undefined"!=typeof window&&window.ENVIRONMENT?window.ENVIRONMENT:"undefined"!=typeof process&&process.env?.NODE_ENV?process.env.NODE_ENV.toUpperCase():"PROD",isDev="DEV"===getEnvironment();"PROD"===getEnvironment();const BASE_API_URL=isDev?"https://dev-api.modochats.com":"https://api.modochats.com",BASE_WEBSOCKET_URL=isDev?"wss://dev-api.modochats.com/ws":"wss://api.modochats.com/ws",VERSION="0.2",NEW_MESSAGE_AUDIO_URL="https://modochats.s3.ir-thr-at1.arvanstorage.ir/new-message.mp3";
...
class CustomerData{_uniqueId;_userData;modo;phoneNumber;constructor(e,t){this.modo=e,this.initializeUniqueId(),this.updateUserData(t),this.initializePhoneNumber();}initializePhoneNumber(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-phone-number`);e&&(this.phoneNumber=e);}initializeUniqueId(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-unique-id`);e?this._uniqueId=e:(this._uniqueId=crypto.randomUUID(),this.savePhoneNumber());}get uniqueId(){return this._uniqueId}get userData(){return this._userData||{}}async updateUserData(e){console.log("newUserData",e),e&&"object"==typeof e?(this._userData=e,await this.fetchUpdate()):console.warn("Invalid user data");}hasSubmittedPhoneForm(){return Boolean(this.phoneNumber)}savePhoneNumber(e){this.phoneNumber=e||"no phone number",localStorage.setItem(`modo-chat:${this.modo.publicKey}-user-phone-number`,e||"no phone number");}async fetchUpdate(){await fetchUpdateUserData(this.modo.publicData?.setting.uuid,this.uniqueId,this.userData);}}

class ModoChat{container;publicKey;publicData;customerData;conversation;socket;options;openedCount=0;version;isInitialized=false;isOpen=false;constructor(t,o){this.publicKey=t,this.customerData=new CustomerData(this,o?.userData),this.version=VERSION,this.options={position:o?.position||"right",theme:o?.theme||"dark",primaryColor:o?.primaryColor||"#667eea",title:o?.title||"",userData:o?.userData,foregroundColor:o?.foregroundColor||"#fff"},o?.autoInit&&this.init();}async init(){if(this.isInitialized)throw new Error("ModoChat already initialized");const t=await fetchModoPublicData(this.publicKey);if(this.publicData=new ModoPublicData(t),!checkIfHostIsAllowed(this))throw new Error("host not allowed");await loadCss(),window.modoChatInstance=()=>this,createChatContainer(this),applyModoOptions(),loadStarters(),updateChatToggleImage(),updateChatTitle(),loadConversation(this),this.isInitialized=true;}async onOpen(){this.isOpen=true,this.openedCount++,this.conversation?.hideTooltip(),this.conversation?.markAsRead(),this.conversation?.scrollToBottom(),1===this.openedCount&&(await(this.conversation?.loadMessages()),await initSocket(),await this.customerData.fetchUpdate());}onClose(){this.isOpen=false;}async updateUserData(t){await this.customerData.updateUserData(t);}}window.ModoChat=ModoChat;

...
const getEnvironment=()=>"undefined"!=typeof window&&window.ENVIRONMENT?window.ENVIRONMENT:"undefined"!=typeof process&&process.env?.NODE_ENV?process.env.NODE_ENV.toUpperCase():"PROD",isDev="DEV"===getEnvironment();"PROD"===getEnvironment();const BASE_API_URL=isDev?"https://dev-api.modochats.com":"https://api.modochats.com",BASE_WEBSOCKET_URL=isDev?"wss://dev-api.modochats.com/ws":"wss://api.modochats.com/ws",VERSION="0.1";
...
const fetchModoPublicData=async e=>await $fetch(`/v1/chatbot/public/${e}`),fetchSendMessage=async(e,t,a,s,o)=>await $fetch("/v2/conversations/website/send-message/",{method:"POST",body:{chatbot_id:e,content:t,message_type:0,unique_id:a,conversation_id:s||void 0,meta_data:{url:window?.location?.href,title:document?.title},phone_number:o&&"no phone number"!==o?o:void 0}}),fetchGetAccessTokenForSocket=async(e,t,a)=>await $fetch("/v2/conversations/websocket/auth/",{method:"POST",body:{chatbot_id:e,conversation_uuid:t,unique_id:a}}),fetchConversationMessages=async(e,t)=>await $fetch(`/v2/conversations/website/conversations/${e}/chatbot/${t}/messages/`),fetchUpdateUserData=async(e,t,a)=>await $fetch("/v1/chatbot/customners/set-user-data",{method:"POST",body:{chatbot_uuid:e,unique_id:t,user_data:a}}),fetchReadMessage=async e=>await $fetch(`/v2/conversations/messages/${e}/`,{method:"POST"});
...
class Conversation{id;uuid;chatbot;unreadMessageCount;messages=[];status;uniqueId;constructor(e){this.id=e.id,this.uuid=e.uuid,this.chatbot=e.chatbot,this.unreadMessageCount=e.unread_messages_count,this.uniqueId=e.unique_id,this.setStatus(e.status),this.onInit();}addMessage(e){this.messages.push(new ConversationMessage(e));const s=document.querySelector(".mc-chat-messages-con");if(s){const t=document.createElement("div"),a=this.messages[this.messages.length-1],o=new Date(a.createdAt).toLocaleTimeString("fa-IR",{hour:"2-digit",minute:"2-digit",hour12:false}),i=new IntersectionObserver(e=>{e[0]?.isIntersecting&&a.fetchRead();},{threshold:.1});t&&i.observe(t),t.innerHTML=`\n        <div class="mc-chat-message ${"USER"===a.type?"mc-chat-message-user":"mc-chat-message-supporter"}">\n          <div class="mc-message-content">${k.parse(e.content)}</div>\n        </div>\n        <div class="mc-message-time">${o}</div>\n      `,t.className="mc-message-wrapper "+("USER"===a.type?"mc-message-wrapper-user":"mc-message-wrapper-supporter"),s.appendChild(t),this.scrollToBottom();}}addSystemMessage(e){const s=document.querySelector(".mc-chat-messages-con");if(s){const t=document.createElement("div");t.className="mc-system-message",t.innerHTML=`\n        <div class="mc-system-message-content">\n          ${e}\n        </div>\n      `,s.appendChild(t),this.scrollToBottom();}}scrollToBottom(){const e=document.querySelector(".mc-chat-messages-con");e&&(e.scrollTop=e.scrollHeight);}clear(){this.messages=[];const e=window.modoChatInstance?.();localStorage.removeItem(`modo-chat:${e?.publicKey}-conversation-uuid`);const s=document.querySelector(".mc-chat-messages-con");s&&(s.innerHTML=""),switchToStarterLayout();}onInit(){switchToConversationLayout(),this.status&&setConversationType(this.status);}setStatus(e){switch(e){case "ai_chat":this.status="AI_CHAT",setConversationType("AI_CHAT");break;case "supporter_chat":this.status="SUPPORTER_CHAT",setConversationType("SUPPORTER_CHAT");break;case "resolved":this.status="RESOLVED";break;default:this.status="UNKNOWN";}}async loadMessages(){const e=window.modoChatInstance?.(),s=await fetchConversationMessages(this.uuid,e?.publicKey);this.messages=[];const t=e?.container?.querySelector(".mc-chat-messages-con");t&&(t.innerHTML="");for(const e of s.results)this.addMessage(e);}}class ConversationMessage{id;content;type;createdAt;isRead=false;constructor(e){switch(this.id=e.id,this.content=e.content,this.isRead=e.is_read||false,e.message_type){case 0:this.type="USER";break;case 1:this.type="AI";break;case 2:this.type="SUPPORTER";break;case 3:this.type="SYSTEM";break;default:this.type="UNKNOWN";}this.createdAt=e.created_at;}fetchRead(){ false===this.isRead&&"USER"!==this.type&&(this.isRead=true,fetchReadMessage(this.id));}}
...
class CustomerData{_uniqueId;_userData;modo;phoneNumber;constructor(e,t){this.modo=e,this.initializeUniqueId(),this.initializeUserData(t),this.initializePhoneNumber();}initializePhoneNumber(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-phone-number`);e&&(this.phoneNumber=e);}initializeUniqueId(){const e=localStorage.getItem(`modo-chat:${this.modo.publicKey}-user-unique-id`);e?this._uniqueId=e:(this._uniqueId=crypto.randomUUID(),this.saveUniqueIdToLocalStorage());}initializeUserData(e){e&&(this._userData=e);}get uniqueId(){return this._uniqueId}get userData(){return this._userData||{}}updateUniqueId(e){e&&""!==e.trim()?this._uniqueId=e.trim():this._uniqueId=crypto.randomUUID(),this.saveUniqueIdToLocalStorage();}updateUserData(e){this._userData=e,this.fetchUpdate();}saveUniqueIdToLocalStorage(){localStorage.setItem(`modo-chat:${this.modo.publicKey}-user-unique-id`,this._uniqueId);}hasSubmittedPhoneForm(){return Boolean(this.phoneNumber)}savePhoneNumber(e){this.phoneNumber=e||"no phone number",localStorage.setItem(`modo-chat:${this.modo.publicKey}-user-phone-number`,e||"no phone number");}fetchUpdate(){"object"==typeof this._userData?fetchUpdateUserData(this.modo.publicData?.setting.uuid,this.uniqueId,this.modo.customerData.userData):console.warn("no user data to update");}}

class ModoChat{container;publicKey;publicData;customerData;conversation;socket;options;openedCount=0;version;isInitialized=false;constructor(t,o){this.publicKey=t,this.customerData=new CustomerData(this,o?.userData),this.version=VERSION,this.options={position:o?.position||"right",theme:o?.theme||"dark",primaryColor:o?.primaryColor||"#667eea",title:o?.title||"",userData:o?.userData,foregroundColor:o?.foregroundColor||"#fff"},o?.autoInit&&this.init();}async init(){if(this.isInitialized)throw new Error("ModoChat already initialized");const t=await fetchModoPublicData(this.publicKey);if(this.publicData=new ModoPublicData(t),!checkIfHostIsAllowed(this))throw new Error("host not allowed");await loadCss(),createChatContainer(this),window.modoChatInstance=()=>this,applyModoOptions(),loadStarters(),updateChatToggleImage(),updateChatTitle(),this.isInitialized=true;}async onOpen(){this.openedCount++,1===this.openedCount&&checkIfUserHasConversation(this);}updateUserData(t){this.customerData.updateUserData(t);}}window.ModoChat=ModoChat;

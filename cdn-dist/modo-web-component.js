...
class Conversation{id;uuid;chatbot;unreadMessageCount;messages=[];status;uniqueId;constructor(s){this.id=s.id,this.uuid=s.uuid,this.chatbot=s.chatbot,this.unreadMessageCount=s.unread_messages_count,this.uniqueId=s.unique_id,this.setStatus(s.status),this.onInit();}addMessage(s){this.messages.push(new ConversationMessage(s));const e=document.querySelector(".mc-chat-messages-con");if(e){const t=document.createElement("div"),a=this.messages[this.messages.length-1],o=new Date(a.createdAt).toLocaleTimeString("fa-IR",{hour:"2-digit",minute:"2-digit",hour12:false});t.innerHTML=`\n        <div class="mc-chat-message ${"USER"===a.type?"mc-chat-message-user":"mc-chat-message-supporter"}">\n          <div class="mc-message-content">${d.parse(s.content)}</div>\n        </div>\n        <div class="mc-message-time">${o}</div>\n      `,t.className="mc-message-wrapper "+("USER"===a.type?"mc-message-wrapper-user":"mc-message-wrapper-supporter"),e.appendChild(t),this.scrollToBottom();}}addSystemMessage(s){const e=document.querySelector(".mc-chat-messages-con");if(e){const t=document.createElement("div");t.className="mc-system-message",t.innerHTML=`\n        <div class="mc-system-message-content">\n          ${s}\n        </div>\n      `,e.appendChild(t),this.scrollToBottom();}}scrollToBottom(){const s=document.querySelector(".mc-chat-messages-con");s&&(s.scrollTop=s.scrollHeight);}clear(){this.messages=[];const s=window.modoChatInstance?.();localStorage.removeItem(`modo-chat:${s?.publicKey}-conversation-uuid`);const e=document.querySelector(".mc-chat-messages-con");e&&(e.innerHTML=""),switchToStarterLayout();}onInit(){switchToConversationLayout(),this.status&&setConversationType(this.status);}setStatus(s){switch(s){case "ai_chat":this.status="AI_CHAT",setConversationType("AI_CHAT");break;case "supporter_chat":this.status="SUPPORTER_CHAT",setConversationType("SUPPORTER_CHAT");break;case "resolved":this.status="RESOLVED";break;default:this.status="UNKNOWN";}}async loadMessages(){const s=window.modoChatInstance?.(),e=await fetchConversationMessages(this.uuid,s?.publicKey);this.messages=[];const t=s?.container?.querySelector(".mc-chat-messages-con");t&&(t.innerHTML="");for(const s of e.results)this.addMessage(s);}}class ConversationMessage{id;content;type;createdAt;constructor(s){switch(this.id=s.id,this.content=s.content,s.message_type){case 0:this.type="USER";break;case 1:this.type="AI";break;case 2:this.type="SUPPORTER";break;case 3:this.type="SYSTEM";break;default:this.type="UNKNOWN";}this.createdAt=s.created_at;}}
...
const sendMessage=async e=>{if(e.trim().length){if(!checkIfUserHasUniqueId())throw new Error("message not send , user uniqueId not found");{const o=window.modoChatInstance?.();try{if(o){if(o?.conversation?.uuid){o.conversation.addMessage({id:"temp",content:e,message_type:0,created_at:(new Date).toISOString()});const t=o.container?.querySelector(".mc-chat-input");t&&(t.value="");}const t=await fetchSendMessage(o?.publicData?.setting.chatbot,e,o?.userData.uniqueId,o?.conversation?.uuid);o?.conversation?.uuid||(o.conversation=new Conversation(t.conversation),o.conversation?.addMessage(t),localStorage.setItem(`modo-chat:${o.publicKey}-conversation-uuid`,o.conversation?.uuid),await initSocket(),"AI_CHAT"===o.conversation.status&&await o.conversation.loadMessages());}else console.error("ModoChat instance not found");}catch{}}}},checkIfUserHasUniqueId=()=>{const e=window.modoChatInstance?.();return !!e?.userData.uniqueId||(e?.conversation?.uniqueId?(e.userData.uniqueId=e.conversation.uniqueId,true):void switchToUniqueIdFormView())},switchToUniqueIdFormView=()=>{const e=window.modoChatInstance?.().container?.querySelector(".mc-form-overlay");e&&(e.classList.remove("mc-hidden"),e.classList.add("mc-active"));},submitUniqueIdForm=e=>{if(PhoneNumberRegex.test(e)){const o=window.modoChatInstance?.();if(o){o.userData.uniqueId=e,localStorage.setItem(`modo-chat:${o.publicKey}-user-unique-id`,e);const t=o.container?.querySelector(".mc-form-overlay");t&&(t.classList.remove("mc-active"),t.classList.add("mc-hidden")),o.container?.querySelector(".mc-send-message-btn")?.click();}else console.error("ModoChat instance not found");}else alert("لطفا شماره تلفن خود را وارد کنید.");};

const registerListeners=e=>{let t=e.querySelector(".mc-chat-body");const n=e.querySelector(".mc-toggle-chat-btn");let r=false;const s=e.querySelector(".mc-footer-link");s&&(s.href=`https://modochats.com?utm_source=${encodeURIComponent(window.location.origin)}`),n.addEventListener("click",()=>{const e=window.modoChatInstance?.();r=!r,r&&e?.onOpen(),t?.classList.toggle("mc-hidden"),n.classList.toggle("mc-chat-open",r);},{capture:false}),registerSendMessageListener(e),registerUniqueIdFormListeners(e),registerNewConversationListener(e);},registerSendMessageListener=e=>{const t=e.querySelector(".mc-chat-input"),n=e.querySelector(".mc-send-message-btn");let r=false;function s(){r=!r,t.disabled=r,n.disabled=r,n.setAttribute("data-is-loading",String(r));}t.addEventListener("keydown",e=>{if("Enter"===e.key){e.preventDefault();const n=t.value;s(),sendMessage(n).then(()=>{t.value="";}).finally(s);}}),n.addEventListener("click",e=>{e.preventDefault();const n=t.value;s(),sendMessage(n).then(()=>{t.value="";}).finally(s);});},registerUniqueIdFormListeners=e=>{const t=e.querySelector(".mc-form-overlay"),n=e.querySelector(".mc-phone-input"),r=e.querySelector(".mc-form-submit-btn"),s=e.querySelector(".mc-form-cancel-btn");r.addEventListener("click",()=>{const e=n.value;submitUniqueIdForm(e);}),s.addEventListener("click",()=>{t.classList.add("mc-hidden");});},registerNewConversationListener=e=>{e.querySelector(".mc-new-conversation-btn").addEventListener("click",()=>{const e=window.modoChatInstance?.();e?.conversation?.clear(),e?.socket?.close(),e&&(e.conversation=void 0,e.socket=void 0);});};

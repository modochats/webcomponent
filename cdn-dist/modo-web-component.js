...
class Socket{socket=null;token;isConnected=false;constructor(e){this.token=e,this.connect();}forceClosed=false;connect(e=false){const t=window.modoChatInstance?.(),s=`${BASE_WEBSOCKET_URL}/conversations/${t?.conversation?.uuid}/messages/?token=${this.token}`;this.socket=new WebSocket(s),this.socket.addEventListener("open",()=>{this.isConnected=true,this.updateConnectionStatus(true),this.socket?.send(JSON.stringify({type:"join_messages"})),e&&t?.conversation?.loadMessages();}),this.socket.onmessage=e=>{const t=JSON.parse(e.data);this.onMessage(t);},this.socket.onclose=()=>this.onclose();}updateConnectionStatus(e){const t=window.modoChatInstance?.(),s=t?.container?.querySelector(".mc-connection-status");s&&(s.className="mc-connection-status "+(e?"mc-connected":"mc-disconnected"));}onMessage(e){const t=window.modoChatInstance?.();switch(e.type){case "new_message":if("USER"===new ConversationMessage(e.message).type)return;t?.conversation?.addMessage(e.message);break;case "ai_response":t?.conversation?.addMessage(e.message);break;default:console.log("unknown message type :",e);}}close(){const e=window.modoChatInstance?.();this.forceClosed=true,this.isConnected=false,this.updateConnectionStatus(false),this.socket?.close(),localStorage.removeItem(`modo-chat:${e?.publicKey}-conversation-access-token`);}onclose(){this.isConnected=false,this.updateConnectionStatus(false),false===this.forceClosed&&setTimeout(()=>{this.connect(true);},3e3);}}const initSocket=async()=>{const e=window.modoChatInstance?.();if(e){const t=localStorage.getItem(`modo-chat:${e.publicKey}-conversation-access-token`);let s="";if(t&&(s=t),!t){const t=await fetchGetAccessTokenForSocket(e.publicData?.setting.uuid,e.conversation?.uuid,e.userData.uniqueId);localStorage.setItem(`modo-chat:${e.publicKey}-conversation-access-token`,t.access_token),s=t.access_token;}e.socket=new Socket(s);}};

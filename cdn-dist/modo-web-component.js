...
const sendMessage=async e=>{if(e.trim().length){if(!checkIfUserHasUniqueId())throw new Error("message not send , user uniqueId not found");{const o=window.modoChatInstance?.();try{if(o){if(o?.conversation?.uuid){o.conversation.addMessage({id:"temp",content:e,message_type:0,created_at:(new Date).toISOString()});const t=o.container?.querySelector(".chat-input");t&&(t.value="");}const t=await fetchSendMessage(o?.publicData?.setting.chatbot,e,o?.userData.uniqueId,o?.conversation?.uuid);o?.conversation?.uuid||(o.conversation=new Conversation(t.conversation),o.conversation?.addMessage(t),localStorage.setItem(`modo-chat:${o.publicKey}-conversation-uuid`,o.conversation?.uuid),await initSocket(),"AI_CHAT"===o.conversation.status&&await o.conversation.loadMessages());}else console.error("ModoChat instance not found");}catch{}}}},checkIfUserHasUniqueId=()=>{const e=window.modoChatInstance?.();return !!e?.userData.uniqueId||(e?.conversation?.uniqueId?(e.userData.uniqueId=e.conversation.uniqueId,true):void switchToUniqueIdFormView())},switchToUniqueIdFormView=()=>{const e=window.modoChatInstance?.().container?.querySelector(".form-overlay");e&&(e.classList.remove("hidden"),e.classList.add("active"));},submitUniqueIdForm=e=>{if(PhoneNumberRegex.test(e)){const o=window.modoChatInstance?.();if(o){o.userData.uniqueId=e,localStorage.setItem(`modo-chat:${o.publicKey}-user-unique-id`,e);const t=o.container?.querySelector(".form-overlay");t&&(t.classList.remove("active"),t.classList.add("hidden")),o.container?.querySelector(".send-message-btn")?.click();}else console.error("ModoChat instance not found");}else alert("لطفا شماره تلفن خود را وارد کنید.");};

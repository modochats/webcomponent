...
const initMessageElement: MessageRelatedUtil = message => {
  ...
  const timeString = messageTime.toLocaleTimeString(widget?.options.language === "fa" ? "fa-IR" : "en-US", {
    hour: "2-digit",
    minute: "2-digit",
    hour12: false
  });
    ...
    repliedToHtml = `
        <div class="mw-replied-to-preview" data-reply-message-id="${message.repliedTo.id}">
          <div class="mw-replied-to-header">
            <span class="mw-replied-to-sender">${message.repliedTo.type === "USER" ? t("chat.repliedTo.sender.user") : t("chat.repliedTo.sender.supporter")}</span>
          </div>
  ...
  if (message.fileSrc) {
    ...
    filePreviewHtml = `
        <a href="${message.fileSrc}" target="_blank" rel="noopener noreferrer" class="mw-file-preview" title="${t("chat.filePreview.downloadTitle")}">
  ...
  element.innerHTML = `
    ...
    ${filePreviewHtml}
      ...
      ${
        ...
        <button class="mw-feedback-btn mw-feedback-dislike" data-message-id="${message.id}" title="${t("chat.feedback.dislike")}">
        ...
        <button class="mw-feedback-btn mw-feedback-like" data-message-id="${message.id}" title="${t("chat.feedback.like")}">
